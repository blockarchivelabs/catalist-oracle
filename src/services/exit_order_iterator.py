...
class ExitOrderIterator:
    ...
    exitable_catalist_validators: list[CatalistValidator]
    ...
    @duration_meter()
    def __iter__(self) -> Iterator[tuple[NodeOperatorGlobalIndex, CatalistValidator]]:
        """
        Prepare state of queue for the iteration: form a queue with Catalist exitable validators
        ...
        self.exitable_catalist_validators = eois.get_exitable_catalist_validators()
        # Prepare dict of node operators stats to sort exitable validators
        self.catalist_node_operator_stats = eois.prepare_catalist_node_operator_stats(self.blockstamp, self.chain_config)
        ...
        self.total_predictable_validators_count = eois.get_total_predictable_validators_count(
            self.blockstamp,
            self.catalist_node_operator_stats
        )
        ...
        self.staking_module_id = {
            ...
            for operator in self.w3.catalist_validators.get_catalist_node_operators(self.blockstamp)
        }
        return self

    @duration_meter()
    def __next__(self) -> tuple[NodeOperatorGlobalIndex, CatalistValidator]:
        """
        ...
        if not self.exitable_catalist_validators:
            raise StopIteration

        self.exitable_catalist_validators.sort(key=self._predicates)
        to_exit = self.exitable_catalist_validators.pop(0)
        global_index = self._decrease_node_operator_stats(to_exit)
        self.left_queue_count += 1
        return global_index, to_exit

    def _decrease_node_operator_stats(self, validator: CatalistValidator) -> NodeOperatorGlobalIndex:
        """
        ...
        before = NodeOperatorPredictableState(**dataclasses.asdict(self.catalist_node_operator_stats[global_index]))
        self.catalist_node_operator_stats[global_index].predictable_validators_count -= 1
        self.catalist_node_operator_stats[global_index].predictable_validators_total_age -= get_validator_age(
        ...
        logger.debug(
            {
                ...
                'before': before,
                'after': self.catalist_node_operator_stats[global_index],
            }
        )
        return global_index

    # -- Predicates for sorting validators --
    def _predicates(self, validator: CatalistValidator) -> tuple:
        global_index = ExitOrderIterator.operator_index_by_validator(self.staking_module_id, validator)
        operator_stats = self.catalist_node_operator_stats[global_index]
        return (
    ...
    @staticmethod
    def _validator_index(validator: CatalistValidator) -> int:
        return int(validator.index)

    @staticmethod
    def operator_index_by_validator(
        staking_module_id: dict[ChecksumAddress, StakingModuleId], validator: CatalistValidator
    ) -> NodeOperatorGlobalIndex:
        return (
            StakingModuleId(staking_module_id[validator.catalist_id.moduleAddress]),
